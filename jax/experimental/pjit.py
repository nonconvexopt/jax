...
def _pjit_partial_eval(trace, *in_tracers,
  ...
  if num_residuals:
    executable = _pjit_lower(**known_params).compile(allow_propagation_to_outputs = True)
...
def unflatten_superdims(assignment):
  ...
  while flat_assignment.size > 1:
    ...
    check(np.array_equal(np.unique(flat_assignment), np.arange(flat_assignment.size)))
  return dims
...
def parse_op_sharding(op_sharding, mesh):
  if op_sharding.type == xc.OpSharding.Type.TUPLE:
    ...
    for dim_size in shape:
      ...
      partitions.append(tuple(reversed(dim_partitions)))
